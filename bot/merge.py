...
async def merge_video_files(file_list, user_id, mode, message):
    """
    ...
    list_file_path = generate_concat_list(file_list, user_id)
    ...
    if mode == 1:
        ...
        logger.info(f"User {user_id}: Starting Hardsub Merge (Encoding)")
        encoder = await get_best_encoder()
        logger.info(f"User {user_id}: Using encoder {encoder}")
        
        # Simple hardsub method (using first file as sub source if available)
        sub_path = str(list_file_path).replace('\\', '/')
        
        # Optimization flags
        cmd += ["-vf", f"subtitles='{sub_path}'"]
        
        if encoder == "h264_nvenc":
            cmd += ["-c:v", "h264_nvenc", "-preset", "p1", "-tune", "ll", "-rc", "constqp", "-qp", "23"]
        elif encoder == "h264_qsv":
            cmd += ["-c:v", "h264_qsv", "-preset", "veryfast", "-global_quality", "23"]
        elif encoder == "h264_amf":
            cmd += ["-c:v", "h264_amf", "-quality", "speed"]
        else:
            # Software fallback with ultrafast
            cmd += ["-c:v", "libx264", "-crf", "23", "-preset", "ultrafast", "-threads", "0"]
